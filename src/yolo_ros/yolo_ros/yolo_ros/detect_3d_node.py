...
class Detect3DNode(LifecycleNode):
    ...
    def process_detections(
        ...
        if bool(self.get_parameter("use_tf").value):
            ...
            if transform is None:
                # TF가 없으면 그냥 카메라 프레임(=depth_info header frame) 기준으로 publish/서비스 응답
    ...
    def convert_bb_to_3d(self, depth_image: np.ndarray, depth_info: CameraInfo, detection: Detection) -> BoundingBox3D:
        ...
        self.get_logger().debug(
            f"3D conversion: u={u_center}, px={px:.1f}, fx={fx:.1f}, "
            f"cam_z={cam_z:.3f}m -> robot_x={robot_x:.3f}m, robot_y={robot_y:.3f}m, robot_z={robot_z:.3f}m"
        )

        msg = BoundingBox3D()
        msg.center.position.x = float(robot_x)  # 전방
        msg.center.position.y = float(robot_y)  # 좌우
        msg.center.position.z = float(robot_z)  # 위아래

        # 크기 변환 (카메라 좌표계 → 로봇 좌표계)
        w = z * (float(depth_w) / fx)  # 카메라 X축 (오른쪽)
        h = z * (float(depth_h) / fy)  # 카메라 Y축 (아래쪽)
    ...
    def on_service(self, request, response):
        ...
        try:
            ...
            # exact match
            if key_q in self._target_cache:
                found = self._target_cache[key_q]
            else:
                # substring match
                if bool(self.get_parameter("match_substring").value):
                    for k, v in self._target_cache.items():
                        if key_q in k or k in key_q:
                            found = v
                            break
            ...
            self.get_logger().info(f"[Service] Sent position for '{query}': X={x:.2f}, Y={y:.2f}, Z={z:.2f}")
            return response
    ...
    def convert_keypoints_to_3d(
        ...
        keypoints_2d = keypoints_2d.astype(np.int16)
